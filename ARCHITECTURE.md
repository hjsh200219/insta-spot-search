...
| # | 레이어 | 구현 위치 | 하는 일 |
|---|--------|-----------|---------|
| 0 | Setup / Preflight | `scripts/setup.py` → `cmd_check()` / `cmd_install()` / `cmd_json()`, `_check_binaries()`, `_brew_pkgs()` | `yt-dlp`/`ffmpeg`/`ffprobe` 존재 확인. 성공 시 무음(exit 0), 없으면 exit 2. macOS 자동설치(`brew install`)는 조회 전용 경계를 벗어난 host mutation이므로 `--yes` 또는 TTY `[y/N]` 동의가 있어야 실행된다 |
...
| 5 | Report | `main()` 후반, `write_manifest()` | `report.json`(**schema_version 1** — `source_access`/`video_path`/`work_dir_owned`/`status`/`warnings`/`audio.*` 포함) + 사람용 stdout 리포트 생성(외부 원문은 `UNTRUSTED CONTENT`로 표시). 이번 실행이 만든 파일 목록을 `.insta-spot-manifest.json`에 기록 |
...
- `setup.py` · `ingest.py` · `lookup.py`는 **서로 import하지 않는 독립 CLI
  진입점**이다. 셋 다 `if __name__ == "__main__"`로 실행되며, 공유 파이썬
  모듈이 없다(그래서 `_check_binaries()`/`check_binaries()`가 각자 구현됨 —
  tech-debt 참고).
...
| 종료 코드 계약 | `ingest.py` `die(code,msg)` | `0` ok · `2` 바이너리 없음 / 사용법 오류 / **cleanup 경계 위반** · `3` 로그인 벽(쿠키 재시도도 실패) · `4` 다운로드·프로브 실패(timeout 포함) · `5` 프레임 추출 실패(timeout·빈 프레임 포함) 또는 워크스페이스 쓰기 실패. SKILL.md "Failure modes" 표와 1:1 매핑 |
| `lookup.py` 종료 코드 | `lookup.py` `die(code,msg)` | `0` ok · `2` 사용법/검증 오류(스킴·크기·MIME·경로 이스케이프) · `4` 네트워크/HTTP 실패. `ingest.py`와 별개의 작은 계약 |
...
| 경로 안전 | `glob.escape()`(`download`·`extract_frames`), `_resolve_created()`/`_resolve_dest()`(경로 이스케이프·심링크 방어), 리스트 인자 subprocess | 모든 glob은 `glob.escape(dir)`로 감싸 특수문자 경로 방어. subprocess는 전부 리스트 인자 — `shell=True` 없음 |
