...
python3 skills/insta-spot-search/scripts/ingest.py <workdir>/video.mp4 \
  --start 6 --end 9 --resolution 2048 --out-dir <workdir>/zoom   # 줌 재추출
...
# smoke: 로컬 영상 하나로 인제스트가 report.json/프레임을 만드는지 확인
```

주요 옵션: `--max-frames N` `--resolution W` `--fps F` `--comments N`
`--profile-scan N` `--no-audio` `--cookies-browser chrome|safari|firefox|edge|brave|none`
...
- **구조화 exit code가 계약** — `0/2/3/4/5`(ingest), `0/2`(setup). `die(code,msg)`로 종료. SKILL.md "Failure modes" 표와 매핑 — 코드 바꾸면 표도 갱신.
