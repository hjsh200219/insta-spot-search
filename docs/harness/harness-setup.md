...
소스 4개 파일(독립 CLI 진입점 `setup.py`·`ingest.py`·`lookup.py` 3개 + 셋이 로컬
import하는 공유 헬퍼 `_common.py` 1개)과 `SKILL.md`짜리 소규모 스킬 리포다. 체크리스트도
그에 맞춰 **가볍게, 그러나 실재하는 계약만** 담는다("지도지 핸드북이 아니다").
...
> 운영 인프라를 설치할 수 있다 — **현재 미설치**. 소스 4파일·package.json 없는 리포라
...
| 자산 | 위치 | 재사용 지침 |
|------|------|-------------|
| exit code 계약 `0/2/3/4/5`(ingest), `0/2`(setup) | `ingest.py:die()`(실체는 `_common.die()`), SKILL.md 표 | 새 코드 만들지 말고 이 5개 안에서 매핑. → [layer-rules §5](../design-docs/layer-rules.md) |
...
| `die(code,msg)` | **`_common.py`**(34–37) | 공유 exit 헬퍼. `ingest.py`·`lookup.py` 둘 다 `from _common import die`로 재사용(중복 해소 — 아래 참고) |
| `REQUIRED_BINARIES` | **`_common.py`**(31) | 필요 바이너리 목록. `setup.py`·`ingest.py` 둘 다 여기서 import. 바이너리 요구사항 변경은 여기 한 곳 |
| `missing_binaries(binaries)` | **`_common.py`**(40–45) | `shutil.which` 순회 헬퍼. `setup.py:_status()`/`ingest.py:check_binaries()`가 각자 리스트를 조립해 이 함수 하나를 호출 |
| `cookie_retry_attempts(cookies_browser)` | **`_common.py`**(48–58) | yt-dlp 쿠키 재시도 사다리. `ingest.py:download()`·`scan_profile()` 둘 다 재사용(중복 해소 — 아래 참고) |
| `PathEscape` / `resolve_within(base, target)` | **`_common.py`**(61–97) | 경로 봉쇄(이스케이프·심링크 방어) 단일 구현. `ingest.py`의 `_remove_stale()`/`cleanup()`과 `lookup.py`의 `_resolve_dest()` 전부 이걸 통해 위임(중복 해소 — 아래 참고) |
...
### 통합 완료 (consolidated into `_common.py` — DONE)

감사에서 나온 중복은 전부 `skills/insta-spot-search/scripts/_common.py`로 합쳐졌다(선제
리팩터가 아니라 각 코드를 실제로 만졌을 때 처리 — 소규모 리포의 마찰을 늘리지 않는 원칙
그대로).

- **쿠키 재시도 패턴** — DONE. `cookie_retry_attempts(cookies_browser)`(`_common.py`
  48–58)로 통합. `ingest.py:download()`·`scan_profile()` 둘 다 이 함수 하나를 호출한다.
- **바이너리 체크 이중 구현** — DONE. `REQUIRED_BINARIES`+`missing_binaries()`(`_common.py`
  31, 40–45)로 통합. `setup.py`·`ingest.py` 두 진입점이 **공유 모듈**을 import한다
  (진입점끼리 import하는 게 아니라 [layer-rules §1](../design-docs/layer-rules.md)이
  허용하는 경로 그대로).
- **`die()` 이중 정의** — DONE. `_common.py:die()`(34–37) 하나로 통합. `ingest.py`·
  `lookup.py` 둘 다 자체 정의를 지우고 여기서 import한다.
- **경로 봉쇄 로직 2종** — DONE. `_common.py:resolve_within()`(67–97)가 두 알고리즘을
  통일해 하나로 재구현했다(둘 중 더 엄격한 쪽의 상위집합). `lookup.py:_resolve_dest()`는
  이제 이 함수에 위임하는 wrapper이고, `ingest.py`의 `_remove_stale()`/`cleanup()`도
  직접 호출한다.
...
- **`logger.ts` / 로깅 모듈(`logging.py`) — 만들지 않음.** 이 리포는 상태·경고를
  ...
  내보내고, Simplicity-First·stdlib-only를 강제한다. 소스 4파일(`setup.py`·`ingest.py`·
  `lookup.py`·`_common.py`)짜리 스킬에 로거 모듈을 추가하는 것은 **요청되지 않은 단일
  사용 추상화**이자 불변식 위반이므로 넣지 않는다.
